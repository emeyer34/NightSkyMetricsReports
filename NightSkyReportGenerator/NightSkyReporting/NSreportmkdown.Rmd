...
avestarvis <- percent(mean(as.numeric(repdata$VISSTARS_RATIO, na.rm = TRUE)))
...
avenelm <- mean(as.numeric(repdata$ZLM, na.rm = TRUE))
...
tab1 <- repdata %>%
  select(MID_DATE_LMT, SITE_NAME, CAMERA, DSET, OBSERVERS) %>% 
...
tab4 <- repdata %>%
  select(MID_DATE_LMT, MID_TIME_LMT, HORIZ_MLX, MAXVERT_MLX, ZENITH_LUM_MSA, VISSTARS_RATIO, ALR_POS) %>%
  arrange(MID_DATE_LMT) %>%
...
tab4 <- tab4 %>%
  select(Date, hhmm, HORIZ_MLX, MAXVERT_MLX, ZENITH_LUM_MSA, VISSTARS_RATIO, ALR_POS)
...
ft4 <- compose(ft4, i = 1, j = 5, part = "header", value = as_paragraph("SQM", "\n (mag/arcsec", as_sup("2"), ")"))
...
tab5 <- repdata %>%
  select(MID_DATE_LMT, BORTLE, ZLM, SQM) %>% # Verify correct measures
  arrange(MID_DATE_LMT) %>%
  mutate_if(is.numeric, round, 2)
...
appenda <- repdata %>%
  select(DNIGHT, MID_DATE_LMT, SITE_NAME, NARRATIVE, FULL_LINK, ART_LINK) %>%
  arrange(MID_DATE_LMT) %>%
