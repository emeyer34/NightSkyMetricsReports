...
# temporarily comment out and replace parameters below for troubleshooting
...
# Determine text description based on the count of values
zenith_count <- sum(!is.na(repdata$ZENITH_LUM_MSA))
avebrightvar <- ifelse(zenith_count == 0, "**~not measured**", 
                ifelse(zenith_count == 1, "from a single measurement", 
                "average"))
...
avestarvisvar <- ifelse(stars_count == 0, "**~not measured**", 
                ifelse(stars_count == 1, "from a single measurement", 
                "on average"))
...
# Classify ALR based on Duriscoe 2018 standards, but these terms are a translation of that paper and first appeared in the Chaco night sky report 
alrcat <- ifelse(aveALRdec <= 0.3, "excellent", ifelse(aveALRdec <= 2.0, "degraded", "significantly degraded"))


###SQM
# Step 1: Replace blank strings with NA in SQM
repdata$SQM[repdata$SQM == " "] <- NA

# Step 2: Count of non-NA values in SQM
sqm_count <- sum(!is.na(repdata$SQM))

# Average SQM description
avesqmvar <- ifelse(sqm_count == 0, "were not taken during this study **~[Delete following text and clarify no SQM taken]**", 
             ifelse(sqm_count == 1, "included a single measurement of", 
             "averaged"))

# Check the current values in SQM
print(unique(repdata$SQM))

# Step 3: Convert SQM to numeric, forcing non-numeric to become NA
repdata$SQM <- as.numeric(as.character(repdata$SQM))

# Step 4: Calculate the mean while excluding NA values
avesqm <- mean(repdata$SQM, na.rm = TRUE)

# Step 5: Format the output to two decimal places
...
# Convert to numeric, suppress warnings about NAs introduced by coercion
repdata$BORTLE <- as.numeric(as.character(repdata$BORTLE))

# Calculate mode
bortle <- mfv(repdata$BORTLE, na_rm = TRUE)

# If there's a tie, return the average rounded to the nearest whole number
if (length(bortle) > 1) {
...
avenelmvar <- ifelse(nelm_count == 0, "**~[NELM was not taken so delete this or clarify]**", 
                ifelse(nelm_count == 1, "single measurement taken of", 
                "average"))
...
soiyear <- as.numeric(sub(".*,\\s*", "", soidate))
...
:::{custom-style="sr Figure caption"}
Figure 3. Panoramic image of the night sky from the `r (soi)` site on `r (soidate)`.  The image is oriented, from left to right, **[NOTE: Author needs to reshuffle the orientation/direction based on the image]: east (90$^\circ$), south (180$^\circ$), west (270$^\circ$), north (0$^\circ$) and east. West is at the center of the image.** The grid lines are spaced 30$^\circ$ apart. The observed sky shows light from all sources, both natural and artificial (`r (fullpath)`). 
:::
...
```{r fig4, echo=FALSE, ft.align="center", out.width='100%',dpi=300}
...
colnames(refdata)<- c("Site Name","Date","Time\n (hh:mm)","Horizontal\n Illuminance \n (mlx)", "Max. Vertical \n Illuminance \n (mlx)", "Zenith \n Luminance \n (mag/arcsec2)", "Stars \n Visible \n  (%)", "All-sky Light \n Pollution \n Ratio")
...
tab5<-repdata %>% select(SITE_NAME, MID_DATE_LMT, BORTLE, ZLM, SQM,Date) %>% #Need to verify correct measures
  arrange(Date)%>%
  mutate_if(is.numeric, round, 2)%>%
  select(-Date)
...
#change column names of reference data set to match tab4 dataset
colnames(refdatabc)<-  c("Site Name","Date","Bortle\n Class","Limiting\n Magnitude", "SQM \n (mag/arcsec2)")
...
for (i in seq_along(appenda$DNIGHT)) {
  ...
  cat("\n") # Additional line breaks for clarity
  cat(paste("  - ","Estimated artificial skyglow:", appenda$ART_LINK[i], "\n\n"))
}
