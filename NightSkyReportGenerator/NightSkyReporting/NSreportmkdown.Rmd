...
packages = c("EnvStats", "reshape2",
             ...
             "kableExtra","statip","magrittr","flextable","officedown", "officer","forstringr")
...
```{r coverimage, echo=FALSE, ft.align="center",out.width = '100%'}
...
###Start gathering data summaries for NPMaps Query data
### most of the metrics reported are here
#import full dataset
data<-read_excel(".//Reference//NPMapsQuery.xlsx")
...
#count of#count of#count of survey nights
daynum<-repdata%>%count(DNIGHT)
daynum<-sum(daynum$n)
  
#earliest date
edate<-min(repdata$MID_DATE_LMT)

#last date
ldate<-max(repdata$MID_DATE_LMT)

#average zenith luminence(brightness) text to indicate in the report if there are averages or single measurement
avebrightvar<-as.data.frame(ifelse(count(repdata)==0, "not measured",ifelse(count(repdata)==1, "a single measurement", ifelse(count(repdata)>1, "the average", "**CHECK VAR**"))))
avebrightvar<-avebrightvar[1,1]
#average zenith luminence(brightness)
avebright<-format(round(mean(repdata$ZENITH_LUM_MSA),2),nsmall=2)

##average star vis text to indicate in the report if there are averages or single measurement
avestarvisvar<-as.data.frame(ifelse(count(repdata)==0, "not measured",ifelse(count(repdata)==1, "a single measurement", ifelse(count(repdata)>1, "the average", "**CHECK VAR**"))))
avestarvisvar<-avestarvisvar[1,1]
#average star visibility
avestarvis<-percent(mean(repdata$VISSTARS_RATIO, na.rm=TRUE))

#average ALR whole sky
...
#most frequent class NEED TO CHECK TO SEE IF THIS IS GOOD
aveALRdec<-mean(repdata$ALR_POS, na.rm=TRUE)
#ALR to descriptive based on Duriscoe
alrcat<-ifelse(aveALRdec <= 0.3, "excellent", ifelse(aveALRdec > 0.3 & aveALRdec <= 2.0, "impaired", "not readily visible"))

#create text variable for report
avesqmvar<-as.data.frame(ifelse(count(repdata)==0, "not measured",ifelse(count(repdata)==1, " single measurement", ifelse(n_distinct(repdata)>1, "average", "CHECK VAR"))))
avesqmvar<-avesqmvar[1,1]
#average SQM
avesqm<-format(round(mean(repdata$SQM, na.rm=TRUE), 2))

# number of datasets
sumdset<-sum(repdata$DSET, na.rm=TRUE)
###End gathering data summaries for NPMaps Query data


###DATA WRANGLING: BortleClass descriptions dataset
#returns most frequent value
bortle<-mfv(repdata$BORTLE)
#read in bortle class table with descriptions
bortletable<-read_excel(".//Reference//BortleClass.xlsx")
# filter by most common bortle value
bortletable<-bortletable %>%
  filter(class==bortle,
         )
#store associated category
...
#create text variable for report
avenelmvar<-as.data.frame(ifelse(n_distinct(repdata$ZLM)==0, "not measured",ifelse(n_distinct(repdata$ZLM)==1, " single measurement", ifelse(n_distinct(repdata$ZLM)>1, "average", "CHECK VAR"))))
avenelmvar<-avenelmvar[1,1]

#average NELM
avenelm<-mean(repdata$ZLM, na.rm=TRUE)
#import nelm table
nelmcat<-read_excel(".//Reference//NELM.xlsx")
#filter to average nelm
nelmcat<-nelmcat %>%
  mutate(diffnelm=abs(avenelm-nelm)) %>%#absolute of the average and categorical nelm
  arrange(diffnelm) %>%  
  slice(1)
#get nelm description and store
...
###List all park codes. Should normally be just one but some examples of two like SEKI (both sequoia and kings managed as one unit)
pmodlist<-unique(alrmodel$UNIT_CODE)

#categorical description model value
alrmodel$cat<-ifelse(alrmodel$MEAN <= 0.3, "excellent", ifelse(alrmodel$MEAN > 0.3 & alrmodel$MEAN <= 2.0, "impaired", "not readily visible"))
#get lowest value in model
...
# What is the most common color within the park based on ALR map symbology and the median ALR of park
alrsymb$maincol<-ifelse(alrmodmed > alrsymb$alrlow & alrmodmed <= alrsymb$alrhigh, "TRUE", "FALSE")
#store the color
parkmodelcolor<-alrsymb%>%
  filter(maincol==TRUE)%>% #filter based on previous ifelse (median value between symbology colors)
 select(color)#reduce down to color
parkcolor<-parkmodelcolor$color


###Start gathering data summaries for light pollution source data
##Must go through arcgis process to generate near table first!
sources<-read_excel(paste(".//Parks//",pcode,"//ReportPark_NearTable_",pcode,".xlsx", sep=""))
sources$NAME<-gsub(' city', '', sources$NAME)
sources$NAME<-gsub(' City', '', sources$NAME)
sources$NAME<-gsub(' CDP', '', sources$NAME)

#convert distance in meters to km
sources$dist_km<-round(sources$NEAR_DIST*.001,digits=0)
#calculate d in walkers law
sources$d<-sources$dist_km^-2.5
#calculate walkers law
sources$walkers<-round((0.01*sources$P0010001*sources$d)*100,digits = 2)
#correct azimuth from arcpro nearest angle calculation
sources$azimuth<-ifelse(sources$NEAR_ANGLE>0, sources$NEAR_ANGLE, (360+sources$NEAR_ANGLE))
sources$azimuth<-round(sources$azimuth, digits=0)
##Now subset data based on site of interest
lightsource<-sources%>%
...
The sky overhead remains **~[insert manual description]** with an `r (avebrightvar)` zenith brightness of `r (avebright)` mag/arcsec^2^. We estimate more than `r (avestarvis)` of stars were still visible for the most of the time, providing an **~[insert manual description]** opportunity to observe the natural night sky from the park. The whole sky over `r (pcode)` is `r (minALR)`-`r (maxALR)` (mean=`r (aveALR)`) brighter than average natural levels, indicating `r (alrcat)` dark sky conditions on average.
...
for (i in 1:length(lightsource$NAME)) {
            cat(paste(lightsource$NAME[i],", ", sep=""))
...
**airmass:** A measure of the amount of air through which light from outside the atmosphere must pass to reach the observer. It is expressed as a ratio of the atmospheric thickness at the zenith. Mathematically, the airmass equals the secant of the zenith angle.
...
**luminance:** The photometric measure of luminous intensity per unit area of a surface. It describes the amount of light that passes through or is emitted from a particular area, and falling within a solid angle. Luminance is often measured in candela per square meter (cd/m2) or lamberts (L).
...
**seeing:** A measure of the optical steadiness of the air, usually judged by looking at the scintillation of stars or by measuring the size of a point source in the image.
...
**vertical illuminance:** Illuminance striking an upright oriented surface, such as a wall or a piece of paper held up to the light illuminance is a photometric measure of luminous light incident on a unit surface area. The derived SI unit of illuminance is the lux (lx).

**zenith:** The point on the celestial sphere directly overhead.

**zodiacal light:** A faint, smooth, and elongated swath of light visible in the night sky. The zodiacal light appears as a noticeable cone of light near the sun, most visible immediately after evening twilight in the west and immediately before morning twilight in the east. It is caused by sunlight reflected off the dust particles in orbit around the sun.
...
include_graphics(paste(park.folder,"park_geographic_location.png",sep = ""))
```
...
include_graphics(paste(park.folder,"all_skycam.png",sep=""))
```
...
:::{custom-style="sr Alternate text"}
Figure 2: A photograph of a typical National Park Service Night Skies Program camera system. The image displays the camera on a standard tripod setup. The camera consists of a commercial lens, a V-band filter, and a CCD camera. The camera system is mounted on a motorized mount, hooked up to external batteries, and controlled by a computer. In the image, taken at night, the camera is in the foreground. The background includes a mountain landscape, a star-filled sky, prominent visibility of the Milky Way galaxy, and a faint skyglow from anthropogenic light on the horizon.
:::
...
tab1<-repdata %>% select(MID_DATE_LMT, SITE_NAME, CAMERA, DSET, OBSERVERS) %>% 
  arrange(MID_DATE_LMT) %>%
  mutate(Date1 = as.Date(MID_DATE_LMT, format = "%d/%m/%Y"))%>% 
  select(Date1, SITE_NAME, CAMERA, DSET, OBSERVERS)
#remove characters
tab1$OBSERVERS <- sub("\r\n", ", ", tab1$OBSERVERS)
tab1$OBSERVERS <- sub("\n", "", tab1$OBSERVERS)
#change column names for report
...
tab2<-repdata %>% select(SITE_NAME, ELEVATION, LATITUDE, LONGITUDE) %>% 
  arrange(ELEVATION)
tab2<-tab2 %>% group_by(SITE_NAME) %>%
...
#change column tables for table
...
#create dataset for table data
siteloop<-repdata %>% select(SITE_NAME, NPS_UNIT, LATITUDE) %>%
   group_by(SITE_NAME,NPS_UNIT)%>%
  #average out different lat/long from different site visits
  summarize(LATITUDE = mean(LATITUDE))%>% 
  arrange(LATITUDE, by_group=FALSE)

for (i in 1:length(siteloop$SITE_NAME)) {
            cat(paste("### ", siteloop$SITE_NAME[i], sep=""))
            cat("  \n")
            cat(paste(siteloop$SITE_NAME[i]," is located in ", siteloop$NPS_UNIT[i]," **[insert site text here. check appendix A]**",sep="")) 
              cat("  \n") 
  cat("  \n") 
...
include_graphics(paste(park.folder,"walkers.png",sep=""))
```
:::
...
lights for our model input. The simplified model of all-sky artificial skyglow (SALR) uses geographic analysis tools to predict the average artificial luminance over the entire night sky. Specifically, this model is based upon a relation between skyglow brightness and the distance from the
...
# Results
The ground-based observations yield the calibrated panoramic images of the night sky. Figure 3 shows an example of the image product associated with each data set. In Appendix A: Observation Notes and Panoramic Images, we provide a complete gallery of the reference images for each night. We use magnitude per square arcsecond (mag/arcsec^2^) for measuring the sky surface brightness. Magnitude (mag or mags) is a standard unit for measuring the brightness of astronomical objects, and it is in inverted logarithmic scale. A sky surface brightness of 22 mag/arcsec^2^ would be considered pristine, and a sky surface brightness of < 20 mag/arcsec^2^ would be considered greatly deviated from the natural condition. The warmer colors in these images represent brighter skies. Figure 3 shows the observed night sky, which contains light from both natural and artificial sources. Purple and dark blue colors indicate an unpolluted sky, and the Milky Way under the natural condition appears green in this color scheme. Figure 4 shows only the light from artificial sources. Light domes along the horizon from the nearby area are more apparent in this bottom image.
...
#change column names for report
colnames(lightsource)<- c("Place","Population\n in 2020","Distance \n (km)", "Azimuth \n (degree)", "Walker's Value \n (% above natural)")
#format table for word report
...
fig3<-repdata %>% select(DNIGHT,SITE_NAME,FULL_LINK) %>% 
...
for (i in 1:length(lightsource$Place)) {
            cat(paste(lightsource$Place[i],", ", sep=""))
...
fig4<-repdata %>% select(DNIGHT,SITE_NAME,ART_LINK) %>% 
  filter(DNIGHT==soidset)
...
:::{custom-style="sr Alternate text"}
Figure 4. Panoramic image of the night sky from the `r (soi)` site on `r (soidate)`. The top of the image includes a scale depicting sky brightness, which is measured in magnitude per square arcsecond (mag/arcsec²). Magnitude (mag) is a common unit for measuring the brightness of astronomical objects, and it is an inverted logarithmic scale. Lower values are brighter. A difference of 5 magnitudes corresponds to a 100x difference in brightness. The images are shown in false color, with warmer colors representing brighter skies. The upper right-hand corner displays the national park unit, site name, and date and time. The night sky image is domed in shape and expands within the frame of the panoramic image. There are grid lines that are spaced 30^∘ apart and they depict the azimuth outward from the camera (0-360 $^\circ$). This image shows only the skyglow from artificial sources, by subtracting out all colors from natural sources. All natural light sources appear dark, which is the **~update text: predominant color in the image**. The grid lines are spaced 30$^\circ$ apart. The image displays estimated skyglow from artificial sources. The natural light has been removed from this image, showing only the artificial skyglow and associated light domes from **[NOTE: Include observations of one or more sources from the following list: ]**
...
We summarize the sky quality measurements in Table 4 and Table 5. Based on the observed panoramic images, we report five indicators (Duriscoe, 2016) that focus on different aspects of sky brightness. These are horizontal illuminance, maximum vertical illuminance, zenith brightness, % of stars visible, and all-sky light pollution ratio (ALR). If multiple data sets were taken at night, we would report the measurements from the set taken under the best observing conditions and free of processing issues. The listed local date and time mark the midpoint of image acquisition. We provide bright urban sky measurements at Rock Creek Park in Washington, DC for comparison. The median natural sky (Duriscoe, 2016) is the natural reference condition against which we can compare all measured values. We complement our CCD camera observations a with visual assessment of Bortle Class and naked eye limiting magnitude (NELM) and take readings with a Unihedron Sky Quality Meter (SQM) whenever possible. A description for each of these metrics is provided below. 
:::
...
tab4<-repdata %>% select(MID_DATE_LMT, MID_TIME_LMT, HORIZ_MLX, MAXVERT_MLX, ZENITH_LUM_MSA, VISSTARS_RATIO, ALR_POS) %>% 
  arrange(MID_DATE_LMT)%>%
...
tab4$Date<-as.character(as.Date(tab4$MID_DATE_LMT))
...
tab4<-tab4 %>% select(Date, hhmm, HORIZ_MLX, MAXVERT_MLX, ZENITH_LUM_MSA, VISSTARS_RATIO, ALR_POS)  

#change column names for published table
colnames(tab4)<- c("Date","Time\n (hh:mm)","Horizontal\n Illuminance \n (mlx)", "Max. Vertical \n Illuminance \n (mlx)", "Zenith \n Brightness \n (mag/arcsec)", "Stars \n Visible \n  (%)", "All-sky Light \n Pollution \n Ratio")

#create dataset for reference values
refdata<- data.frame(date=c('Urban Sky', 'Natural Sky'),Time=c('' , ''), hi=c(39.56,0.80), mvi=c(29.04, 0.40), zb=c(18.00, 22.00), ls=c(8,100), alr=c(64.43,0.00) )
#refdata$zb<-(round(refdata$zb, digits=2))

#change column names of reference data set to match tab4 dataset
colnames(refdata)<- c("Date","Time\n (hh:mm)","Horizontal\n Illuminance \n (mlx)", "Max. Vertical \n Illuminance \n (mlx)", "Zenith \n Brightness \n (mag/arcsec)", "Stars \n Visible \n  (%)", "All-sky Light \n Pollution \n Ratio")
...
#add superscript 2
ft4c<-compose(ft4b, i=1, j=5,part="header", value=as_paragraph( "SQM","\n (mag/arcsec",as_sup("2"),")"))
...
tab5<-repdata %>% select(MID_DATE_LMT, BORTLE, ZLM, SQM) %>% #Need to verify correct measures
  arrange(MID_DATE_LMT)%>%
  mutate_if(is.numeric, round, 2) 

#convert to better date format
tab5$MID_DATE_LMT<-as.character(as.Date(tab5$MID_DATE_LMT))
...
#create dataset for reference values
refdatabc<- data.frame(date=c('Urban Sky', 'Natural Sky'),bc=c(8,1), lm=c(5.2, 7.0), sqm=c(18.36, 22.00))
#refdata$zb<-(round(refdata$zb, digits=2))

#change column names of reference data set to match tab4 dataset
colnames(refdatabc)<-  c("Date","Bortle\n Class","Limiting\n Magnitude", "SQM \n (mag/arcsec2)")
...
#add superscript 2
ft5c<-compose(ft5b, i=1, j=4,part="header", value=as_paragraph( "SQM","\n (mag/arcsec",as_sup("2"),")"))
#print table
ft5c
...
#(MID_DATE_LMT, MID_TIME_LMT, HORIZ_MLX, MAXVERT_MLX, ZENITH_ART_MAGS, VISSTARS_RATIO, ALR_POS)
maxvert<-round(max(repdata$MAXVERT_MLX),digits=2)
maxvertref<-ifelse(maxvert< 0.40, "less than", ifelse(maxvert == 0.40, "equal to", "greater than"))
zenskyb<-round(ave(repdata$ZENITH_LUM_MSA),digits=2)
```
...
for (i in 1:length(top3cities$NAME)) {
            cat(paste(top3cities$NAME[i],", ", sep=""))
...
for (i in 1:length(pmodlist)) {
  alrmod<-subset(alrmodel, alrmodel$UNIT_CODE==pmodlist[i])
            cat(paste("At ", alrmod$UNIT_CODE, ", the mean modeled ALR was ", round(alrmod$MEAN, digits=2)," (", round(alrmod$MIN,digits = 2)," - ", round(alrmod$MAX, digits=2),"). ","At this average ALR, the modeled whole-park sky quality condition is considered ", alrmod$cat,". ", sep=""))
            }
...
Practicing sustainable outdoor lighting within the park and with the neighboring communities is a key to reducing light pollution. Sustainable outdoor lighting observes the following principles: light only if needed, light only when needed, light only where it is needed, use warm-white or amber light, use the minimum amount of light needed, and use energy-efficient lights. Locally, appropriate lighting in the park can provide visitors the optimal observing environment. Partnering with gateway communities can further reduce light pollution, both locally and regionally. Reducing light pollution improves the quality of night sky viewing. Ultimately, improving night sky quality can result in increased visitor spending in gateway communities. Preserving the natural night sky really requires a joint effort and can be mutually beneficial to all parties.
:::
\newpage
...
—. 2016. Photometric indicators of visual night sky quality derived from all-sky brightness maps. Journal of Quantitative Spectroscopy and Radiative Transfer. 181, 33
...
#create dataset for table data and fromat on survey date
appenda<-repdata %>% select(DNIGHT,MID_DATE_LMT, SITE_NAME, NARRATIVE,FULL_LINK,ART_LINK) %>% 
  arrange(MID_DATE_LMT) %>%
  mutate(Date = as.Date(MID_DATE_LMT, format = "%d/%m/%Y"))
appenda$NARRATIVE<-gsub("NARRATIVE: ","",appenda$NARRATIVE)


for (i in 1:length(appenda$DNIGHT)) {
            cat(paste("On ", appenda$Date[i], ", at ",appenda$SITE_NAME[i],":", appenda$NARRATIVE[i], sep=""))
            cat("  \n")
