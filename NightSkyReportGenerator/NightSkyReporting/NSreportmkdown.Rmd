...
avestarvis <- percent(mean(as.numeric(repdata$VISSTARS_PCT, na.rm = TRUE)))
...
tab4 <- repdata %>%
  select(MID_DATE_LMT, MID_TIME_LMT, HORIZ_MLX, MAXVERT_MLX, ZENITH_LUM_MSA, VISSTARS_PCT, ALR_POS) %>%
  arrange(MID_DATE_LMT) %>%
...
tab4 <- tab4 %>%
  select(Date, hhmm, HORIZ_MLX, MAXVERT_MLX, ZENITH_LUM_MSA, VISSTARS_PCT, ALR_POS)
